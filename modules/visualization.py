...
class VisualizationPageComplete:
    ...
    def render_data_setup(self):
        ...
        # Έλεγχος για διαθέσιμα δεδομένα (multiple possible keys)
        data_sources = []
        available_data = {}
        
        # Έλεγχος για προεπεξεργασμένα δεδομένα
        if 'preprocessed_adata' in st.session_state and st.session_state['preprocessed_adata'] is not None:
            ...
            available_data["Προεπεξεργασμένα Δεδομένα"] = st.session_state['preprocessed_adata']
        elif 'adata' in st.session_state and st.session_state.adata is not None:
            data_sources.append("Προεπεξεργασμένα Δεδομένα")
            available_data["Προεπεξεργασμένα Δεδομένα"] = st.session_state.adata
        
        # Έλεγχος για integrated δεδομένα
        if 'integrated_adata' in st.session_state and st.session_state['integrated_adata'] is not None:
            data_sources.append("Integrated Δεδομένα")
            available_data["Integrated Δεδομένα"] = st.session_state['integrated_adata']
        elif 'integrated_data' in st.session_state and st.session_state.integrated_data is not None:
            data_sources.append("Integrated Δεδομένα")
            available_data["Integrated Δεδομένα"] = st.session_state.integrated_data
        
        if not data_sources:
            st.warning("⚠️ Δεν βρέθηκαν διαθέσιμα δεδομένα.")
            st.info("💡 Πηγαίνετε πρώτα στο Preprocessing ή Integration module.")
            
            # Debug info
            with st.expander("🔍 Debug: Available Session Keys"):
                available_keys = [key for key in st.session_state.keys() if 'adata' in key.lower()]
                st.write("Available keys:", available_keys)
            return
        ...
        # Φόρτωση επιλεγμένων δεδομένων
        self.adata = available_data[selected_source]
    ...
    def _prepare_data_for_visualization(self, adata, max_cells: int = 15000, max_genes: int = 5000):
        """Προετοιμασία δεδομένων για γρήγορη visualization με διατήρηση ποιότητας"""
        
        st.info(f"🎨 Βελτιστοποίηση για visualization: {adata.n_obs:,} κύτταρα, {adata.n_vars:,} γονίδια")
        
        # Smart cell sampling για visualization
        if adata.n_obs > max_cells:
            st.info(f"📊 Subsampling: {adata.n_obs:,} → {max_cells:,} κύτταρα για γρήγορη visualization")
            
            # Stratified sampling αν υπάρχουν ομάδες
            if 'dataset' in adata.obs.columns:
                # Proportional sampling ανά dataset
                datasets = adata.obs['dataset'].unique()
                sampled_indices = []
                
                for dataset in datasets:
                    dataset_mask = adata.obs['dataset'] == dataset
                    dataset_indices = np.where(dataset_mask)[0]
                    n_dataset_cells = len(dataset_indices)
                    
                    # Proportional sampling
                    proportion = n_dataset_cells / adata.n_obs
                    target_cells = max(100, int(proportion * max_cells))
                    target_cells = min(target_cells, n_dataset_cells)
                    
                    if target_cells < n_dataset_cells:
                        selected = np.random.choice(dataset_indices, target_cells, replace=False)
                    else:
                        selected = dataset_indices
                    
                    sampled_indices.extend(selected)
                    st.info(f"  - {dataset}: {len(selected):,} κύτταρα")
                
                viz_adata = adata[sampled_indices, :].copy()
            else:
                # Random sampling
                indices = np.random.choice(adata.n_obs, max_cells, replace=False)
                viz_adata = adata[indices, :].copy()
        else:
            viz_adata = adata.copy()
        
        # Smart gene selection για PCA
        if viz_adata.n_vars > max_genes:
            st.info(f"🧬 Gene selection: {viz_adata.n_vars:,} → {max_genes:,} γονίδια")
            
            # Προτεραιότητα σε highly variable genes
            if 'highly_variable' in viz_adata.var.columns:
                hvg_mask = viz_adata.var['highly_variable']
                hvg_genes = viz_adata.var_names[hvg_mask]
                
                if len(hvg_genes) >= max_genes:
                    # Χρήση μόνο HVG
                    selected_genes = hvg_genes[:max_genes]
                    st.info(f"  - Χρήση {len(selected_genes)} highly variable genes")
                else:
                    # HVG + top expressed genes
                    n_hvg = len(hvg_genes)
                    n_additional = max_genes - n_hvg
                    
                    # Βρίσκουμε top expressed non-HVG genes
                    non_hvg_mask = ~hvg_mask
                    if np.any(non_hvg_mask):
                        if sparse.issparse(viz_adata.X):
                            mean_expr = np.array(viz_adata.X[:, non_hvg_mask].mean(axis=0)).flatten()
                        else:
                            mean_expr = viz_adata.X[:, non_hvg_mask].mean(axis=0)
                        
                        top_expr_idx = np.argsort(mean_expr)[::-1][:n_additional]
                        non_hvg_genes = viz_adata.var_names[non_hvg_mask]
                        additional_genes = non_hvg_genes[top_expr_idx]
                        
                        selected_genes = np.concatenate([hvg_genes, additional_genes])
                        st.info(f"  - {n_hvg} HVG + {len(additional_genes)} top expressed")
                    else:
                        selected_genes = hvg_genes
                
                viz_adata = viz_adata[:, selected_genes].copy()
            else:
                # Fallback: top expressed genes
                if sparse.issparse(viz_adata.X):
                    mean_expr = np.array(viz_adata.X.mean(axis=0)).flatten()
                else:
                    mean_expr = viz_adata.X.mean(axis=0)
                
                top_genes_idx = np.argsort(mean_expr)[::-1][:max_genes]
                viz_adata = viz_adata[:, top_genes_idx].copy()
                st.info(f"  - Top {max_genes} expressed genes")
        
        # Memory optimization
        viz_adata = memory_manager.optimize_adata_for_memory(viz_adata)
        
        st.success(f"✅ Visualization data ready: {viz_adata.n_obs:,} κύτταρα, {viz_adata.n_vars:,} γονίδια")
        ...
        if st.button(f"🚀 Υπολογισμός {method}"):
            try:
                with st.spinner(f"Υπολογισμός {method}..."):
                    ...
                    # ΚΡΙΣΙΜΗ ΒΕΛΤΙΩΣΗ: Mandatory subsampling για visualization
                    if self.plot_data.n_obs > 15000:
                        st.warning(f"⚠️ Subsampling για visualization: {self.plot_data.n_obs:,} → 15,000 κύτταρα")
                        indices = np.random.choice(self.plot_data.n_obs, 15000, replace=False)
                        viz_data = self.plot_data[indices, :].copy()
                    else:
                        viz_data = self.plot_data.copy()
                    
                    # Gene filtering για PCA speed
                    if viz_data.n_vars > 5000:
                        if 'highly_variable' in viz_data.var.columns:
                            hvg_mask = viz_data.var['highly_variable']
                            if hvg_mask.sum() > 0:
                                viz_data = viz_data[:, hvg_mask].copy()
                                st.info(f"🧬 Using {viz_data.n_vars:,} HVG for visualization")
                    
                    st.success(f"✅ Visualization data: {viz_data.n_obs:,} cells, {viz_data.n_vars:,} genes")
                    
                    if method == "UMAP":
                        if 'X_pca' not in viz_data.obsm:
                            st.info("🔄 Optimized PCA για UMAP...")
                            sc.tl.pca(viz_data, n_comps=min(50, viz_data.n_vars-1))
                        sc.pp.neighbors(viz_data, n_neighbors=n_neighbors)
                        sc.tl.umap(viz_data, min_dist=min_dist)
                        basis = 'umap'
                        
                    elif method == "t-SNE":
                        if 'X_pca' not in viz_data.obsm:
                            st.info("🔄 Optimized PCA για t-SNE...")
                            sc.tl.pca(viz_data, n_comps=min(50, viz_data.n_vars-1))
                        sc.tl.tsne(viz_data, perplexity=perplexity, learning_rate=learning_rate)
                        basis = 'tsne'
                        
                    elif method == "PCA":
                        st.info("🔄 Optimized PCA computation...")
                        sc.tl.pca(viz_data, n_comps=min(50, viz_data.n_vars-1))
                        basis = 'pca'
                        
                    else:  # Diffusion Map
                        if 'X_pca' not in viz_data.obsm:
                            st.info("🔄 Optimized PCA για Diffusion Map...")
                            sc.tl.pca(viz_data, n_comps=min(50, viz_data.n_vars-1))
                        sc.pp.neighbors(viz_data)
                        sc.tl.diffmap(viz_data)
                        basis = 'diffmap'
                    
                    # Αποθήκευση optimized data
                    self.plot_data = viz_data
