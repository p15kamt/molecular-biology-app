...
class CellAnnotationPageComplete:
    ...
    def render_data_clustering(self):
        ...
        # Check for integrated data (multiple possible keys)
        ...
        elif 'integrated_adata' in st.session_state and st.session_state['integrated_adata'] is not None:
            data_sources.append("Integrated Δεδομένα")
            available_data["Integrated Δεδομένα"] = st.session_state['integrated_adata']
        
        if not data_sources:
            st.warning("⚠️ Δεν βρέθηκαν δεδομένα από προηγούμενα βήματα")
            st.info("💡 Εκτελέστε πρώτα Preprocessing ή Integration για να φορτώσετε δεδομένα")
        ...
        # Auto-load first available data
        selected_source = data_sources[0]
        self.adata = available_data[selected_source]
        
        st.success(f"✅ Φορτώθηκαν {selected_source}")
        
        # Option to change data source if multiple available
    ...
    def perform_clustering(self):
        """Εκτέλεση clustering με intelligent subsampling"""
        
        try:
            with st.spinner("Εκτέλεση optimized clustering..."):
                
                # Standard clustering with reasonable parameters
    ...
    def render_builtin_markers(self):
        ...
        if selected_category:
            ...
            if st.button(f"✅ Χρήση {selected_category} Markers για Annotation"):
                ...
                # Αποθήκευση στο session state για διατήρηση
                st.session_state['marker_genes'] = self.marker_genes
    ...
    def render_automated_annotation(self):
        ...
        # Έλεγχος και φόρτωση marker genes από session state
        if 'marker_genes' in st.session_state and st.session_state['marker_genes']:
            self.marker_genes = st.session_state['marker_genes']
            st.info(f"✅ Φορτώθηκαν marker genes από session: {list(self.marker_genes.keys())}")
        
        if not self.marker_genes or len(self.marker_genes) == 0:
            st.warning("⚠️ Φορτώστε πρώτα marker genes από το 'Marker Genes' tab")
            st.info("💡 Πηγαίνετε στο tab 'Marker Genes' και επιλέξτε built-in ή upload custom markers")
            
            # Debug info για marker genes
            with st.expander("🔍 Debug: Marker Genes Status"):
                st.write(f"Instance marker genes: {self.marker_genes}")
                st.write(f"Session marker genes: {st.session_state.get('marker_genes', 'Not found')}")
            return
    ...
    def perform_score_based_annotation(self, score_threshold, min_genes):
        ...
        # DEBUG: Marker genes analysis
        st.subheader("🔍 Debug: Marker Genes Analysis")
        
        total_marker_genes = 0
        available_marker_genes = 0
        debug_info = {}
            ...
            for category, cell_types in self.marker_genes.items():
                if isinstance(cell_types, dict):
                    ...
                    for cell_type, markers in cell_types.items():
                        total_marker_genes += len(markers)
                        available_markers = [gene for gene in markers if gene in self.adata.var_names]
                        available_marker_genes += len(available_markers)
                        
                        debug_info[category][cell_type] = {
                            'total': len(markers),
                            'available': len(available_markers),
                            'missing': [gene for gene in markers if gene not in self.adata.var_names]
                        }
                        
                        st.write(f"  - {cell_type}: {len(available_markers)}/{len(markers)} markers available")
                        if len(available_markers) > 0:
                            st.write(f"    ✅ Available: {', '.join(available_markers[:3])}{'...' if len(available_markers) > 3 else ''}")
                        if len(available_markers) < len(markers):
                            missing = [gene for gene in markers if gene not in self.adata.var_names]
                            st.write(f"    ❌ Missing: {', '.join(missing[:3])}{'...' if len(missing) > 3 else ''}")
        
        st.info(f"📊 Summary: {available_marker_genes}/{total_marker_genes} marker genes found in dataset")
        st.info(f"🔧 Parameters: min_genes={min_genes}, score_threshold={score_threshold}")
        
        if available_marker_genes == 0:
            st.error("❌ Κανένα marker gene δεν βρέθηκε στο dataset!")
            return ['Unknown'] * self.adata.n_obs
        
        # Υπολογισμός scores για κάθε κύτταρο
        cell_scores = []
        
        # Progress bar για μεγάλα datasets
        if self.adata.n_obs > 1000:
            st.info(f"🔄 Processing {self.adata.n_obs:,} cells...")
            progress_bar = st.progress(0)
            batch_size = max(100, self.adata.n_obs // 100)
        else:
            progress_bar = None
        ...
        for cell_idx in range(self.adata.n_obs):
            ...
            # Υπολογισμός score για κάθε cell type - FIXED
            if isinstance(self.marker_genes, dict):
                for category, cell_types in self.marker_genes.items():
                    if isinstance(cell_types, dict):
                        for cell_type, markers in cell_types.items():
    ...
    def perform_clustering_based_annotation(self):
        ...
        for cluster in self.adata.obs[cluster_col].unique():
            ...
            # Safe iteration over marker genes
            if isinstance(self.marker_genes, dict):
                for category, cell_types in self.marker_genes.items():
                    if isinstance(cell_types, dict):
                        for cell_type, markers in cell_types.items():
    ...
    def render_annotation_markers(self, annotation_col):
        ...
        relevant_markers = []
        # Safe iteration for marker gene lookup
        if isinstance(self.marker_genes, dict):
            for category, types in self.marker_genes.items():
                if isinstance(types, dict) and selected_cell_type in types:
